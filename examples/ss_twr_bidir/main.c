...
#define TASK_DELAY        /*200*/10           /**< Task delay. Delays a LED0 task for 200 ms */
...
int main(void)
{
  ...
  LEDS_ON(BSP_LED_0_MASK /*| BSP_LED_1_MASK*/ | BSP_LED_2_MASK );
    ...
    //! Using LED1 for Alert Testing
    /* Start timer for LED1 blinking */ //Disabled so we can use it for distance warning
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  // IF WE GET HERE THEN THE LEDS WILL BLINK
    ...
    while (1)
    {
    //!! Yea, this won't work; but, we are using FREERTOS
      ss_init_run();
      ss_resp_run();
    }
